...
RemoveCpG = rownames(mset_2)%in%rownames(beta_mat_3)
mset_3 = mset_2[RemoveCpG,]

rm(mset_2, beta_mat_2, beta_mat_3, rgset_2)
gc()
```
## Renormalisation
...
```{r}
# Alternatively, filter for gene on list by Teo et al. 2019
filt_index <- which(probe_genes_teo$probeID %in% rownames(beta_renorm)) 

# determine list of probes to keep for NMF
filt_probes <- rownames(var_probes[filt_index,])
beta_final_teo <- beta_renorm[filt_probes, ]
