...
# Preprocessing Methylation data  TCGA idat files
...
Using the TCGAbiolinks package, we are querying the data directly from the GDC portal. Since the idat files are only available from the legacy archive, this needs to be specified. More information on how to query projects and data types can be found here:
https://bioconductor.statistik.tu-dortmund.de/packages/3.5/bioc/vignettes/TCGAbiolinks/inst/doc/tcgaBiolinks.html#gdcquery:_searching_gdc_data_for_download
...
```
### Matching idat file names to patient IDs

Because the raw methylation data is differently named than the clinical information we have, we're reading in meta data to match patients with their respective data. So far, we have only downloaded the data; to read it in properly, this step needs to be taken first.
...
Creating matrix of meta data with old filenames based on the Sentrix ID of the assay plate, and the ones we want to replace them with, for easier reading in and processing.
```{r}
...
### Reading in renamed idat files
 To read in the files, we need to provide the file names without the suffix of the probe colour, and only provide unique patient IDs (the are double becuase of green and red probe colours).
```{r}
...
Loading data in as RGChannel set; though we will mostly be working with a MethyLumi set (no particular reason other than that is was easier to handle for me).
```{r}
...
load clinical data in from previous analysis
```{r}
# idat data
...
adjusting the pheno data to only include relevant information
```{r}
...
Loading Methylation data in as MethyLumi object --> easier to work with
```{r}
...
Looking at median intensities of methylated and unmethylated probes to check their distribution. For blood samples, median intensities under 2000 are to be filtered. However, not sure on the procedure for tissue samples - sample signal range seems to be alright.
```{r}
...
```
```{r}
# plot with cutoff lines
par(mfrow = c(1,1))
plot(M.median, U.median, pch = 16, xlab = "Median M intensity", ylab = "Median U intensity", 
     main = "Raw median probe intensity")
abline(v = intens.Thres, col = "red")
abline(h = intens.Thres, col = "red")
```
To already check for potential (technical) confounders, median signal strength was plotted for each assay plate. 
...
boxplot(U.median ~ pheno$sentrix_id, ylab = "Median U intensity", xlab = "Plate", 
        las = 2, col = rainbow(nCol))
```
Especially in the scatterplot a rather clear pattern is visible, some of the patients seem to cluster together based on which plate their samples ran on. This is to be noted and to be checked later on. As these are raw samples for now this effect may be corrected out without additional steps in the normal procedure of preprocessing.
...
```{r}
# Plot for Rmd
hist(bs, xlab = "Median % BS conversion", main = "Bisulfite Conversion rate", col = "lightblue")
...
As this effect seems to be very big already on a pure signal strength level, further analysis will be uniracial caucasian. At the end of the study, findings will be evaluated on their bias towards race & gender.
```{r}
# Density plots
...
Multidimesional scaling, checks the 1000 most variable positions and relates samples. Distance not to be taken absolute. Just to another way to check for confounders at this point, and to overall see data distribution. Pretty clear confounder is sex, as was to be expected.
...
In this step, specific probes in the methylation assay corresponding to common mutation sites (SNPs) and their methylation value are correlated between patients to exclude close family relations (corr <0.09). 
...
```
To check in a different visual way, next is plotted the highest correlation value of any sample with another in a heatmap. There appear to be two clusters which can not quite be accounted for with any of the likely cofactors, but again is is quite clear that samples are not closely related. (For the one sample where this isn't the case: It is the same patient providing tumour tissue sample as well as healthy tissue, which will be filtered for in the next section)
...
gplots::heatmap.2(abs(snpCor), trace = "none", labCol = pheno$barcode, 
                  ...
                  dendrogram = "column", labRow = "", key = T, scale = "none")
```
...
Filtering patients with tissue type "normal" and "recurrent tumour", as well as one patient with missing clinical data. Furthermore, this analysis will only include caucasian patients, as the confounding effect of race was already very visible on the signal strength level and may therefore likely influence downstream analyses heavily.
...
index1 <- which(pheno_11$race == "white")# | pheno_11$race == "black or african american") 
...
As was stated above in the Visualisation part, bisulfite conversion is a simple QC measure for methylation data. QC probes should have a BS conversion rate of 100%; everything below 70% is considered of too low quality here, and samples which are affected by this will be filtered out.
...
The pfilter function filters data sets based on bead count and detection p-values. Samples with low detection rates are removed, as are probe with low detection.
```{r}
# run pfilter on data
mset_2 <- pfilter(mset_2)
```

### Age prediction using wateRmelons age test agep

To further check the qulity of the data, it is usual to predict the age of the samples based on their methylation rate, and compare this with the actual biological age. This is done using Horvath's coefficients.
However, since the method is based on normal tissue, and here we are dealing with cancer, the results are expected to differ, since tumour tissues tends to be hypomethylated (and generally not normally methylated, which will throw off the results).
```{r}
# predict age
...
hist(dnamage[,1], xlab = "DNAmAge", main = "Predicted Age")
plot(y=dnamage[,1], x=pheno_2$age, ylab = "Predicted", xlab = "Reported",xlim=c(0,100),ylim=c(0,200), main = "Age predicted vs reported")
```


## 2.3 Normalisation

WateRmelon's dasen normalisation takes the different types of probes for 450k illumina assays into account. Type I and II probes have been developed at different time points but are still used in the same assay, causing differences in the intensity measurements for the data signals. the dasen normalisation corrects for this and is the recommended approach.
```{r}
...
To check how well the normalisation has performed, we visualise the data as box plots, as density curves, and take a look at the area under the curve for each of the density curves.
```{r}
# Boxplot
...
pairs(project.pca$x[,1:5], col = rainbow(nCol)[group_sentrix], 
      main="Principal components analysis bi-plot\nPCs 1-5 coloured by assay plate", pch=16)
...
To correct for the effect of sex on the data, as well as individual differences, in the next step probes encoding for regions on the X or Y chromosome as well as probes covering known SNP regions will be removed. 

### Interlude - getting rgset up to current status

To access the annotation data with minfi, we need to use an RGChannelSet, or somehting else that minfi can deal with. I realised later that I didnt actually need to filter the samples here, as it only takes the probes (which I am matching afterwards anyway), but I have left this part in still.
...
pairs(project.pca2$x[,1:5], col = rainbow(nCol)[group_sentrix], 
      main="Principal components analysis bi-plot\nPCs 1-5 coloured by assay plate after chr filtering",
      pch=16)
legend(0.05, 0.02, fill = rainbow(nCol),legend = unique(pheno_2$sentrix_id), 
       ncol = 5, text.width = 0.15, cex = 0.7, x.intersp = 0.2)
...
### Prepare lists for filtering based on different criteria

In the next steps, we create sereval criteria for wichi we can do a feature seelction. This includes biologically relevant gene lists, variance, as well as randomly selected genes or probes.
...
```
As an example, we visualise the overlap ov Weronicka's three gene lists with the the data plotted for variance. Can be done for any of the gene-probe lists.
```{r}
# combine probes lists from all three gene lists of Weronicka
...
var_probes[probe_genes_v1$probeID, "colour"] <- "red" #for different lists, change filter input 
...
filt_index <- which(var_probes >=0.04) 

# determine list of probes to keep for NMF
#filt_probes <- rownames(var_probes[filt_index,])
beta_final <- beta_renorm[filt_index, ]

# save beta matrix for further analysis
setwd("C:/Users/Sarah/Documents/Studium/Master/Systems Biology Maastricht/Thesis/R")
write.table(beta_final, "beta_preprocessed_var2.txt", sep="\t",row.names=T)
```

To compare the selected probes and their associated genes in further analyses, we alsofirst extract them and then save them in this next step.
```{r}
# save table with probes & associated genes when filtered for variance
# associated probes with genes for selected probes
...
# write table with all genes that are associated with probes in the 450k assay
...
### 1b) Filtering for random probes

To further check for bias, we will also take a subset of 10.000 random probes, regardless of their association to any gene, to see the possibly different results in NNMF
```{r}
# create index of 10.000 random probes
beta_renorm <- betas(mset_renorm)
rand_idx <- sample(1:dim(beta_renorm)[1], 10000)

# determine list of probes to keep for NMF
beta_final_random_probes <- beta_renorm[rand_idx, ]

# save beta matrix for further analysis
setwd("C:/Users/Sarah/Documents/Studium/Master/Systems Biology Maastricht/Thesis/R")
write.table(beta_final_random_probes, "beta_preprocessed_random_probes.txt", sep="\t",row.names=T)
```


## 2) Filtering based on gene list

Here we filter the probes based on one of the biological gene lists and their associated probes as established above. 
```{r}
# Alternatively, filter for gene lists
# change name of gene list as appropriate (_teo, _nefte, _wang, _wero150)
setwd("C:/Users/Sarah/Documents/Studium/Master/Systems Biology Maastricht/Thesis/R")
probe_genes_teo <- read.csv(file = "probe_genes_teo.csv", header = T, sep = ";")
beta_renorm <- betas(mset_renorm)
filt_index <- which(probe_genes_teo$probeID %in% rownames(beta_renorm)) 
...
# save beta matrix for further analysis
setwd("C:/Users/Sarah/Documents/Studium/Master/Systems Biology Maastricht/Thesis/R")
write.table(beta_final, "beta_preprocessed_teo2.txt", sep="\t",row.names=T)
```

### 2a) Lowest common denominator

As an experiment, we also wanted to run NMF with the lowest common denominator genes found in all biological gene lists, which were only two genes. We associate them here with probes and filter for these.
Realistically, this feature selection gives us too little information because of the low number of features selected, but it was still an interesting experiment.
```{r}
...
```{r}
# Filter beta matrix for overlap
beta_renorm <- betas(mset_renorm)
...
After preprocessing the data and applying adequate feature selection, the last open step is finally saving the beta matrix for the NMF experiments.
```{r}
# save beta matrix for further analysis
setwd("C:/Users/Sarah/Documents/Studium/Master/Systems Biology Maastricht/Thesis/R")
