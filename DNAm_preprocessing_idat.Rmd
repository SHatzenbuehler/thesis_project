...
# Data download
GDCdownload(query_DNAm, method = "client", files.per.chunk = 10)
data.DNAm <- GDCprepare(query_DNAm, summarizedExperiment = F)
...
# load data
data.DNAm <- readRDS(file = "DNAm_data.RDS")
data.DNAm_annot <- readRDS(file = "DNAm_data_table.RDS")
```
...
# genetic data not in summarizedExperiment form:
annot <- data.DNAm_annot[, 1:3]
```

Step 0: Filter out probes on Sex chromosomes

```{r}
chr_val <- as.vector(seqnames(data.DNAm), mode="any")
keep <- !chr_val %in% c("chrX", "chrY")
met0 <- met[keep, ]
```
...
probe.na <- rowSums(is.na(met0))
table(probe.na == 0)
# chose those has not NA values in rows
probe <- probe.na[probe.na == 0]
met1 <- met0[row.names(met0) %in% names(probe), ]
any(is.na(met1))
...
Step 3: filter out rs# probes

```{r}
keep3 <- !str_detect(rownames(met2), regex("rs.", dotall = TRUE))
met3 <- met2[keep3, ]
```
...
met_mat <- as.matrix(as.data.frame(met3))
```

# Visualtisation of data distribution before normalisation
...
# Boxplots
boxplot(met_mat, xlab = "Patients", ylab = "beta", las = 2, main = "Before normalisation")
```

# AUC under density curve

```{r}
# calculate density matrix for each patient
# loop for densities
met_dens <- vector(mode = "list", length = dim(met_mat)[2])
names(met_dens) <- colnames(met_mat)
for (i in colnames(met_mat)){
  met_dens[[i]] =  density(met_mat[,i])
}

# check plot
plot(NULL, xlim=c(-0.1,1.1), ylim=c(0, 5.5), ylab="density", xlab = "beta")
for(i in seq_along(met_dens)) {
     lines(met_dens[[i]]) }

# loop for auc
met_auc <- matrix(, nrow = dim(met_mat)[2], ncol = 1)
rownames(met_auc) <- colnames(met_mat)
for (i in colnames(met_mat)){
  met_auc[i,] =  densityauc(met_dens[[i]], c(0,1))
}

# plot
plot(met_auc, ylim=c(0.5,1), xlab = "patients", ylab="AUC", 
     main = "Area under density curve before normalisation")
```

# QC visualisation for potential cofactors
...
# AUC after BMIQ Normalisation

```{r}
# calculate density matrix for each patient
# loop for densities
met_dens_BMIQ <- vector(mode = "list", length = dim(met_normBMIQ)[2])
names(met_dens_BMIQ) <- colnames(met_normBMIQ)
for (i in colnames(met_normBMIQ)){
  met_dens_BMIQ[[i]] =  density(met_normBMIQ[,i])
}

# check plot
plot(NULL, xlim=c(-0.1,1.1), ylim=c(0, 5.5), ylab="density", xlab = "beta")
for(i in seq_along(met_dens_BMIQ)) {
     lines(met_dens_BMIQ[[i]]) }

# loop for auc
met_auc_BMIQ <- matrix(, nrow = dim(met_normBMIQ)[2], ncol = 1)
rownames(met_auc_BMIQ) <- colnames(met_normBMIQ)
for (i in colnames(met_normBMIQ)){
  met_auc_BMIQ[i,] =  densityauc(met_dens_BMIQ[[i]], c(0,1))
}

# plot
plot(met_auc_BMIQ, ylim=c(0.5,1), xlab = "patients", ylab="AUC", 
     main = "Area under density curve after BMIQ normalisation")
```

Different Normalisation: Quantile
independent of each other?

```{r}
# normalisation
met_betaqn <- betaqn(met_normBMIQ)

# Density plot
densityPlot(met_betaqn, main = "After Quantile Normalisation")
# Boxplot
boxplot(met_betaqn, xlab = "Patients", ylab = "beta", las = 2, main = "After quantile normalisation")
```
# AUC after quantile Normalisation

```{r}
# calculate density matrix for each patient
# loop for densities
met_dens_qn <- vector(mode = "list", length = dim(met_betaqn)[2])
names(met_dens_qn) <- colnames(met_betaqn)
for (i in colnames(met_betaqn)){
  met_dens_qn[[i]] =  density(met_betaqn[,i])
}

# check plot
plot(NULL, xlim=c(-0.1,1.1), ylim=c(0, 5.5), ylab="density", xlab = "beta")
for(i in seq_along(met_dens_qn)) {
     lines(met_dens_qn[[i]]) }

# loop for auc
met_auc_qn <- matrix(, nrow = dim(met_betaqn)[2], ncol = 1)
rownames(met_auc_qn) <- colnames(met_betaqn)
for (i in colnames(met_betaqn)){
  met_auc_qn[i,] =  densityauc(met_dens_qn[[i]], c(0,1))
}

# plot
plot(met_auc_qn, ylim=c(0.5,1), xlab = "patients", ylab="AUC", 
     main = "Area under density curve after quantile normalisation")
```

# Variance Filtering

```{r}
# plotting Variance per probe after both normalisations
plot(rowVars(met_betaqn), xlab = "probes", ylab = "Variance", 
...
library(wateRmelon)
predictedSex <- predictSex(met_normBMIQ)# needs normalising first, uses PCA 
...
```
## PCA After BMIQ normalisation

```{r}
project.pca_norm <- prcomp(t(met_normBMIQ))
project.pca_norm.proportionvariances <- ((project.pca_norm$sdev^2) / (sum(project.pca_norm$sdev^2)))*100

# Bar plot (or Scree Plot)
barplot(project.pca_norm.proportionvariances, cex.names=1, 
        xlab=paste("Principal component (PC), 1-", length(project.pca_norm$sdev)), 
        ylab="Proportion of variation (%)", main="Scree plot after normalisation", ylim=c(0,20))
```

```{r}
# Biplots of the first 10 PCs after normalisation
par(cex=1.0, cex.axis=0.8, cex.main=0.8, xpd = TRUE)
pairs(project.pca_norm$x[,1:5], col = c("red", "cornflowerblue")[group_sex], 
      main="PCs 1-5 coloured by sex \n(Post-Normalisation)", pch=16)
legend("bottomright", fill = c("red", "cornflowerblue"), legend = c("male", "female"))
pairs(project.pca_norm$x[,6:10], col = c("red", "cornflowerblue")[group_sex], 
      main="PCs 6-10 coloured by sex \n(Post-Normalisation)", pch=16)
legend("bottomright", fill = c("red", "cornflowerblue"), legend = c("male", "female"))
```
...
```{r}
## transform UCSC gene names to gene ID list
library(AnnotationDbi)
library(org.Mm.eg.db)

#x <- ann450k$UCSC_RefGene_Name
#rownames(x) <- rownames(ann450k) # add rownames to vector, then this stuff is not necessary

geneSymbols <- select(org.Mm.eg.db, columns = "SYMBOL",  keytype = "REFSEQ")
