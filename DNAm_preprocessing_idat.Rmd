...
To correct for the effect of sex on the data, as well as individual differences, in the next step probes encoding fro regions on the X or Y chromosome as well as porbes covering known SNP regions will be removed. 
...
keep <- !(row.names(beta_mat) %in% ann450k$Name[ann450k$chr %in% c("chrX","chrY")])
table(keep)
...
beta_mat <- beta_mat[row.names(beta_mat) %in% c(no.snp.probe, snp5.probe), ]

#remove no-further needed datasets
rm(no.snp.probe, probe, probe.na, snp.probe, snp5.probe, keep)
```
