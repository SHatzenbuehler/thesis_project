...
# Cross check with already preprocessed beta vlaues provided for same data
...
### 2) Filtering & Data adjustment based on QC assesment so far
...
```{r}
# Create PCA project
project.pca2 <- prcomp(t(beta_mat_2))
...
```{r}
# asign new filtered beta_met2 to mset_3


# normalise again
mset_norm2 <- dasen(mset_3)
```


## 2.7 CpG filtering based on variance and associated genes
...
```{r}
# variance
var_probes <- rowVars(beta_norm)

# plotting Variance per probe after normalisation 2
plot(var_probes, xlab = "probes", ylab = "Variance", 
