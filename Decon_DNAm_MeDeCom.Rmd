...
# Non-negative Matrix Factorisation based on Lutsik et al. (2017): MeDeCom
...
paras.teo <- medecom.result.example@parameters   #change input here
output.teo <- medecom.result.example@outputs     #change input here
...
#plot 3D
plot3d(pca.teo$x[,1:3], xlim = c(min(pred.lmc[,1])-1, max(pred.lmc[,1]+1)), 
     ylim = c(min(pred.lmc[,2])-1, max(pred.lmc[,2]+2)), cex = 2,
     zlim = c(min(pred.lmc[,3])-1, max(pred.lmc[,3]+1)), 
     #main = "PCA of LMC projection filtered for teo", 
     pch = 16,  type = "p",
     xlab = "PC1", ylab = "PC2", zlab = "PC3", decorate = T)
plot3d(pred.lmc[,1:3], col = "red", pch = 16, add = T)
#text(pred.lmc, labels = rownames(pred.lmc), pos = 3)

## Doesnt look that great, I'll leave that
```



### Try and  add previously established groups for patients as colours into PCA
...
#- Set1 for RNA (or Dark2?), Set2 for DNAm
colours_pca <- c(brewer.pal(nlevels(group_type_rna)-1, 'Set1'), "black") # change here
