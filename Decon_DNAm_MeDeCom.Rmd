...
paras.teo <- medecom.result.black_var@parameters   #change input here
output.teo <- medecom.result.black_var@outputs     #change input here
...
pca.teo <- prcomp(t(beta_mat.black_var))         #change input here
...
pairs(pca.teo$x[,1:4], pch=16, xlim = c(min(pred.lmc-1), max(pred.lmc+1)), 
      ylim = c(min(pred.lmc-1), max(pred.lmc+1)), 
            panel=function(x, y, ...) {
                ...
                i <<- i +1},
      main = "Black sample group: PCA of LMC projection filtered for var"
            )
```
...
par(cex.main=.8)
heatmap.2(dist.black1, main = "Black sample group: similarity of LMC proportions", 
...
par(cex.main=.8)
heatmap.2(dist.cau_bl, main = "Caucasion vs Black: LMC profile similarity for teo", 
...
rownames(clinical) <- new_IDs
# find corresponding patients
sample_IDs <- which(rownames(clinical) %in% colnames(beta_mat.teo)) # change unput here
# extract relevant group info
group_type <- clinical$paper_Original.Subtype[sample_IDs]
group_type <- droplevels(group_type)
...
levels(group_type) <- c(levels(group_type),"none")
group_type[is.na(group_type)] <- "none"

# get data proportions
prop.data <- getProportions(medecom.result.teo, # change input here
                            K=4, lambda=0.01) 
#colnames(prop.data) <- colnames(beta_mat.teo)
colnames(prop.data) <- colnames(prop.data, do.NULL = F, prefix = "V_")
```
...
colours_dend <- c(brewer.pal(nlevels(group_type_meth)-1, 'Set1'), "black") # change here
clusters <- as.numeric(factor(group_type_meth, levels = levels(group_type_meth))) # change here
...
# visualise
plot(clusDendro, main = "Dendrogram of LMC proportions \n coloured by DNAm groups",
     cex = 0.6, ylab = "height")
legend("topright", 
     legend = c(levels(group_type_meth)),  # change input here
     col = c(colours_dend),  
